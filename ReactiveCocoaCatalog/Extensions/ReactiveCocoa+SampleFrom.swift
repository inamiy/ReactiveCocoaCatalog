...
extension SignalType {
    ...
    /// Inverse of `sampleOn`, combining receiver's (sampler) and `samplee`'s values.
    @warn_unused_result(message="Did you forget to call `observe` on the signal?")
    public func sampleFrom<Value2>(samplee: Signal<Value2, NoError>) -> Signal<(Value, Value2), Error> {
    ...
    /// Inverse of `sampleOn`, combining receiver's (sampler) and `samplee`'s values.
    @warn_unused_result(message="Did you forget to call `observe` on the signal?")
    public func sampleFrom<Value2>(samplee: SignalProducer<Value2, NoError>) -> Signal<(Value, Value2), Error> {
        return Signal { observer in
...
extension SignalProducerType {
    ...
    public func sampleFrom<Value2>(samplee: SignalProducer<Value2, NoError>) -> SignalProducer<(Value, Value2), Error> {
